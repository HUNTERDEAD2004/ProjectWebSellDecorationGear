...
namespace DecorGearApi.Controllers
{
    ...
    public class ImageListsController : ControllerBase
    {
        ...
        [HttpPut("update")]
        public async Task<IActionResult> UpdateImageLists(ImageListDto request, CancellationToken cancellationToken)
        {
            // Kiểm tra nếu ModelState không hợp lệ
            if (!ModelState.IsValid)
            {
            ...
            var valueId = await _res.GetImageById(request.ImageListID, cancellationToken);
            if (valueId == null)
            {
            ...
            valueId.ImageListID = request.ImageListID;
            valueId.ImagePath = request.ImagePath;
            valueId.ProductID = request.ProductID;
            valueId.ImagePath = request.ImagePath;
            

            // Gọi phương thức Update để lưu các thay đổi
            var result = await _res.UpdateImage(valueId, cancellationToken);
