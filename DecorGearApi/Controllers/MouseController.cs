...
namespace DecorGearApi.Controllers
{
    ...
    public class MouseController : ControllerBase
    {
        ...
        [HttpPut("update")]
        public async Task<IActionResult> UpdateMouse(MouseDetailsDto request, CancellationToken cancellationToken)
        {
            // Kiểm tra nếu ModelState không hợp lệ
            if (!ModelState.IsValid)
            {
            ...
            var valueId = await _res.GetMouseById(request.MouseDetailID, cancellationToken);
            if (valueId == null)
            {
            ...
            valueId.MouseDetailID = request.MouseDetailID;
            valueId.ProductID = request.ProductID;
            valueId.Color = request.Color;
            valueId.DPI = request.DPI;
            valueId.Connectivity = request.Connectivity;
            valueId.Dimensions = request.Dimensions;
            valueId.Material = request.Material;
            valueId.EyeReading = request.EyeReading;
            valueId.Button = request.Button;
            valueId.LED = request.LED;
            valueId.SS = request.SS;

            // Gọi phương thức Update để lưu các thay đổi
            var result = await _res.UpdateMouse(valueId, cancellationToken);
