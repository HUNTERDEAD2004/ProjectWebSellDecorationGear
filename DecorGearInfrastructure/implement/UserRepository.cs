...
namespace DecorGearInfrastructure.Implement
{
    public class UserRepository : IUserRespository
    {
        ...
        public async Task<ResponseDto<bool>> DeleteUser(int id, CancellationToken cancellationToken)
        {
            ...
            // 1. Xóa CartDetails và Carts liên quan
            var carts = await _db.Carts.Where(c => c.UserID == id).ToListAsync(cancellationToken);
            if (carts.Any())
            {
                var cartIds = carts.Select(c => c.CartID).ToList();
                var cartDetails = await _db.CartDetails.Where(cd => cartIds.Contains(cd.CartID)).ToListAsync(cancellationToken);
                if (cartDetails.Any())
                {
            ...
            // 2. Xóa các Member liên quan
            var members = await _db.Members.Where(m => m.UserID == id).ToListAsync(cancellationToken);
            if (members.Any())
            {
                _db.Members.RemoveRange(members);
            }

            // 3. Xóa các Order liên quan
            var orders = await _db.Orders.Where(o => o.UserID == id).ToListAsync(cancellationToken);
            if (orders.Any())
            {
                _db.Orders.RemoveRange(orders);
            }

            // 4. Xóa các Favorite liên quan
            var favorites = await _db.Favorites.Where(f => f.UserID == id).ToListAsync(cancellationToken);
            if (favorites.Any())
            {
                _db.Favorites.RemoveRange(favorites);
            }

            // 5. Xóa các Feedback liên quan
            var feedbacks = await _db.FeedBacks.Where(f => f.UserID == id).ToListAsync(cancellationToken);
            if (feedbacks.Any())
            {
        ...
        public async Task DeleteVerificationCodeAsync(string email, string code, CancellationToken cancellationToken)
        {
            var user = await _db.Users.FirstOrDefaultAsync(u => u.Email == email, cancellationToken);
            if (user != null)
            {
                var verificationCode = await _db.VerificationCodes
                    .FirstOrDefaultAsync(vc => vc.Id == user.UserID && vc.Code == code, cancellationToken);
